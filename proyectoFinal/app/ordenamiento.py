...
def ordenar_por_seleccion(lista_usuarios):
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            if lista_usuarios[j].username < lista_usuarios[min_index].username:
                min_index = j
...
def buscar_usuario_por_dni(usuarios, dni):
    ...
    lista_usuarios = sorted(usuarios.values(), key=lambda usuario: usuario.dni)
    ...
    with open(log_filename, "w") as log_file:
        ...
        while izquierda <= derecha:
            ...
            dni_actual = lista_usuarios[medio].dni
...
def buscar_usuario_por_username(usuarios, username, archivo_ordenado=False):
    ...
    if archivo_ordenado:
        ...
        with open(log_filename, "w") as log_file:
            ...
            while izquierda <= derecha:
                medio = (izquierda + derecha) // 2
                username_actual = lista_usuarios[medio].username
        ...
        for intento, usuario in enumerate(usuarios.values(), 1):
            print(f"Intento {intento}: {username} comparado con {usuario.username}")
...
def buscar_usuario_por_email(usuarios, email):
    ...
    for intento, usuario in enumerate(usuarios.values(), 1):
        print(f"Intento {intento}: {email} comparado con {usuario.email}")
